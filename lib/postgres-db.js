...
async function findEndpoint(endpointHash) {
  const FIND_ENDPOINT = "SELECT * FROM endpoints WHERE endpoint_hash = $1;";
...
async function postEndpoint(newEndpointHash) {
  const MAKE_ENDPOINT =
    "INSERT INTO endpoints (endpoint_hash, endpoint_name) VALUES ($1, $2);";
...
async function postRequest(parsedRequest, newRequestHash) {
  const MAKE_REQUEST = `INSERT INTO requests (endpoint_hash, http_method, http_path, endpoint_id, request_hash) VALUES ($1, $2, $3, $4, $5);`;
