...
const { wss, WebSocket } = require("./servers");
...
async function processRequest(req, res) {
  ...
  wss.clients.forEach(async (client) => {
    const result = await redisStore.all((err, sessions) => {
      console.log("length right here" + sessions.length);
    });
    if (
      client.readyState === WebSocket.OPEN
      // req.session.endpoint === extractedMetaData.endpointHash
    ) {
